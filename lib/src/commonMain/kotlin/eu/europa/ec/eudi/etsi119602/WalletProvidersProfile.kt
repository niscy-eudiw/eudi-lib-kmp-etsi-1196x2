 ...
 */
public object WalletProvidersProfile : Profile {
    override val scheme: Scheme get() = Scheme.EXPLICIT
    override val type: LoTEType get() = LoTEType(ETSI19602.WALLET_PROVIDERS_LOTE_TYPE);
    override val statusDeterminationApproach: String get() = ETSI19602.WALLET_PROVIDERS_STATUS_DETERMINATION_APPROACH
    override val schemeCommunityRules: List<MultiLanguageURI>
        get() = listOf(
            MultiLanguageURI(Language.EN, URIValue(ETSI19602.WALLET_PROVIDERS_SCHEME_COMMUNITY_RULES))
        )
    ...
    private const val PROFILE_NAME = "Wallet Providers"

    public fun ListOfTrustedEntities.ensureScheme() {
        try {
            schemeInformation.ensureWalletProvidersScheme(WalletProvidersProfile)
